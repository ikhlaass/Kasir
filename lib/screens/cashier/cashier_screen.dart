...
class _CashierScreenState extends State<CashierScreen> {
  ...
  Future<void> _closeShift() async {
    ...
    if (shift != null) {
      if (!mounted) return;
      Navigator.push(context, MaterialPageRoute(builder: (_) => CloseShiftScreen(user: widget.user, shift: shift)));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tidak ada shift aktif')));
    }
  }
  ...
  Future<void> _showKasKeluarDialog() async {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Catat Kas Keluar', style: AppFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textDark)),
        content: Form(
          key: formKey,
          child: Column(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                ...
                if (!mounted) return;
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Kas keluar berhasil dicatat'), backgroundColor: AppColors.success));
              }
            },
            child: Text('Simpan', style: AppFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
          ...
          Expanded(
            child: _currentIndex == 0 ? _buildBuatPesananView(isWide) : _buildPendingOrdersView(),
          ),
        ],
      ),
      bottomNavigationBar: isWide ? null : BottomNavigationBar(
        ...
        selectedLabelStyle: AppFonts.poppins(fontWeight: FontWeight.w600, fontSize: 12),
        unselectedLabelStyle: AppFonts.poppins(fontSize: 12),
        items: const [
          BottomNavigationBarItem(icon: Icon(Icons.point_of_sale_outlined), activeIcon: Icon(Icons.point_of_sale), label: 'Kasir'),
          BottomNavigationBarItem(icon: Icon(Icons.receipt_long_outlined), activeIcon: Icon(Icons.receipt_long), label: 'Pesanan'),
          BottomNavigationBarItem(icon: Icon(Icons.print_outlined), label: 'Printer'),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Nav items
          _sidebarItem(0, Icons.point_of_sale_outlined, Icons.point_of_sale_rounded, 'Kasir'),
          _sidebarItem(1, Icons.receipt_long_outlined, Icons.receipt_long_rounded, 'Daftar Pesanan'),
          ...
          if (widget.user.role == 'admin') ...[
            ...
            Padding(
              ...
              child: InkWell(
                onTap: () {
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => AdminDashboardScreen(user: widget.user)));
                },
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
        ...
        Expanded(
          child: Consumer<ProductProvider>(
            builder: (ctx, provider, _) {
              ...
              if (products.isEmpty) {
                return Center(child: Text('Tidak ada menu', style: AppFonts.poppins(color: AppColors.textLight)));
              }
                ...
                child: cart.items.isEmpty
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            Icon(Icons.shopping_cart_outlined, size: 48, color: AppColors.textLight.withValues(alpha: 0.5)),
                            const SizedBox(height: 16),
                            Text('Keranjang masih kosong', style: AppFonts.poppins(color: AppColors.textMedium)),
                          ],
                        ),
                      )
                    : ListView.separated(
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(item.product.namaMenu, style: AppFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14, color: AppColors.textDark)),
                                    const SizedBox(height: 4),
                                    Text('Rp ${item.product.harga.toInt()}', style: AppFonts.poppins(fontSize: 12, color: AppColors.textMedium)),
                                    if (item.catatan?.isNotEmpty == true) ...[
                                      const SizedBox(height: 4),
                                      Text('Catatan: ${item.catatan}', style: AppFonts.poppins(fontSize: 11, color: AppColors.primary).copyWith(fontStyle: FontStyle.italic)),
                                    ],
                                  ],
                                ),
                              ),
                              Row(
                                children: [
                                  _qtyBtn(Icons.remove, () => cart.removeItem(item.product)),
                                  SizedBox(
                                    width: 32,
                                    child: Text(
                                      '${item.qty}',
                                      textAlign: TextAlign.center,
                                      style: AppFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14),
                                    ),
                                  ),
                  ...
                  child: Column(
                    children: [
                      Row(
                        children: [
                          Expanded(
                            child: Text('Total Pembayaran', style: AppFonts.poppins(fontSize: 14, color: AppColors.textMedium)),
                          ),
                          Text('Rp ${cart.totalHarga.toInt()}', style: AppFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                        ],
                      ),
                      const SizedBox(height: 16),
                      Row(
                        children: [
                          Expanded(
                            child: ElevatedButton(
                              ...
                              child: Text('Simpan', style: AppFonts.poppins(fontWeight: FontWeight.w600, fontSize: 13)),
                            ),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            flex: 2,
                            child: ElevatedButton(
                              style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14), elevation: 0),
                              onPressed: () {
                                if (MediaQuery.of(context).size.width < 950) Navigator.pop(context);
                                _showPaymentDialog(context, cart);
                              },
                              child: Text('Bayar Langsung', style: AppFonts.poppins(fontWeight: FontWeight.w600, fontSize: 13)),
                            ),
                          ),
                        ],
                      ),
  ...
  void _confirmClearCart(CartProvider cart) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: AppColors.surface,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
        title: Text('Kosongkan Pesanan?', style: AppFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16, color: AppColors.textDark)),
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text('Kosongkan', style: AppFonts.poppins(fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );
  ...
  void _savePendingOrder(CartProvider cart) async {
    ...
    final details = cart.items.map((item) => TransactionDetailModel(
    ...
    if (mounted) {
      if (MediaQuery.of(context).size.width < 950 && Navigator.canPop(context)) {
  ...
  void _showPaymentDialog(BuildContext context, CartProvider cart) {
    ...
    final uangCtrl = TextEditingController(text: cart.totalHarga.toInt().toString());
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('Total Tagihan', style: AppFonts.poppins(fontSize: 14, color: AppColors.textMedium)),
                        const SizedBox(height: 8),
                        Text('Rp ${cart.totalHarga.toInt()}', style: AppFonts.poppins(fontSize: 32, fontWeight: FontWeight.bold, color: AppColors.textDark, letterSpacing: -1)),
                      ],
                    ),
                  ),
                  ...
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Pilih Metode Pembayaran', style: AppFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
                        const SizedBox(height: 12),
                        Row(
                              ...
                              child: GestureDetector(
                                ...
                                child: AnimatedContainer(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: Border.all(color: isSel ? AppColors.primary : AppColors.border, width: isSel ? 1.5 : 1),
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                  alignment: Alignment.center,
                                  child: Text(m, style: AppFonts.poppins(
                                    fontSize: 13,
                        ...
                        if (isTunai) ...[
                          ...
                          Text('Uang Diterima (Rp)', style: AppFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
                          const SizedBox(height: 12),
                          TextFormField(
                            controller: uangCtrl,
                            keyboardType: TextInputType.number,
                            ...
                            decoration: InputDecoration(
                              prefixText: 'Rp ',
                              prefixStyle: AppFonts.poppins(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textMedium),
                              filled: true,
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: valid ? AppColors.success.withValues(alpha: 0.1) : AppColors.error.withValues(alpha: 0.1),
                              borderRadius: BorderRadius.circular(12),
                            ),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Kembalian', style: AppFonts.poppins(fontSize: 14, color: valid ? AppColors.success : AppColors.error)),
                                Text('Rp ${kembalian > 0 ? kembalian.toInt() : 0}', style: AppFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: valid ? AppColors.success : AppColors.error)),
                              ],
                            ),
                          ),
                        ],
                            ...
                            Expanded(
                              ...
                              child: ElevatedButton(
                                onPressed: valid ? () async {
                                  ...
                                  if (selectedMethod == 'Tunai') {
                                    ...
                                    if (shift != null && shift.id != null) {
                                      await db.updateShiftExpectedCash(shift.id!, cart.totalHarga);
                                    }
                                  }
                                        ...
                                        child: Container(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              const SizedBox(height: 8),
                                              Text('Kembalian: Rp ${kembalian > 0 ? kembalian.toInt() : 0}', style: AppFonts.poppins(fontSize: 14, color: AppColors.textMedium)),
                                              const SizedBox(height: 32),
                                              SizedBox(
                                                ...
                                                child: ElevatedButton.icon(
                                                  onPressed: () async {
                                                    ...
                                                    final printed = await ps.printReceipt(trx, receiptItems, dibayar: uang, kembalian: kembalian);
                                                    if (!printed) {
                                                      if (!sCtx.mounted) return;
                                                      ScaffoldMessenger.of(sCtx).showSnackBar(const SnackBar(
                                              ...
                                              SizedBox(
                                                ...
                                                child: TextButton(
                                                  ...
                                                  child: Text('Tutup', style: AppFonts.poppins(color: AppColors.textMedium, fontWeight: FontWeight.w600)),
                                                ),
                                              ),
                                            ],
                                          ),
                                ...
                                style: ElevatedButton.styleFrom(
                                  padding: const EdgeInsets.symmetric(vertical: 16),
                                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                                  backgroundColor: valid ? AppColors.primary : AppColors.border,
                                ),
                                child: Text('Bayar & Proses', style: AppFonts.poppins(fontWeight: FontWeight.w600, color: valid ? Colors.white : AppColors.textLight)),
                              ),
                            ),
                          ],
                        ),
              ...
              children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(product.namaMenu, style: AppFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                            Text('Rp ${product.harga.toInt()}', style: AppFonts.poppins(color: AppColors.primary, fontWeight: FontWeight.w600)),
                          ],
                        ),
                      ),
                      IconButton(
                ...
                Expanded(
                  child: ListView(
                    padding: const EdgeInsets.all(20),
                    children: [
                      Text('Level Pedas (Pilih 1)', style: AppFonts.poppins(fontWeight: FontWeight.bold, color: AppColors.textMedium)),
                      const SizedBox(height: 12),
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: ['Tidak Pedas', 'Sedang', 'Pedas', 'Sangat Pedas'].map((lvl) {
                          final isSelected = pedasLevel == lvl;
                          return ChoiceChip(
                            label: Text(lvl, style: AppFonts.poppins(color: isSelected ? Colors.white : AppColors.textDark)),
                            selected: isSelected,
                            selectedColor: AppColors.primary,
                            backgroundColor: AppColors.background,
                            onSelected: (v) => setStateSB(() => pedasLevel = lvl),
                          );
                        }).toList(),
                      ),
                      if (extraProducts.isNotEmpty) ...[
                        const SizedBox(height: 24),
                        Text('Extra / Topping', style: AppFonts.poppins(fontWeight: FontWeight.bold, color: AppColors.textMedium)),
                        const SizedBox(height: 12),
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(extra.namaMenu, style: AppFonts.poppins(fontWeight: FontWeight.w600)),
                                      Text('+ Rp ${extra.harga.toInt()}', style: AppFonts.poppins(fontSize: 12, color: AppColors.primary)),
                                    ],
                                  ),
                                ),
                                if (qty == 0)
                                  ElevatedButton(
                                    ...
                                    child: Text('Tambah', style: AppFonts.poppins(fontSize: 12)),
                                  )
                                else
                                  Row(
                                    children: [
                ...
                Padding(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text('Masukkan Keranjang', style: AppFonts.poppins(fontWeight: FontWeight.bold, color: Colors.white)),
                  ),
                ),
              ],
            ),
            ...
            Expanded(
              child: Container(
                ...
                child: product.imagePath == null || !File(product.imagePath!).existsSync()
                    ? Center(
                        child: Icon(
                          product.kategori == 'Minuman' ? Icons.local_drink_outlined : Icons.restaurant_outlined,
                          size: 32,
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    product.namaMenu,
                    style: AppFonts.poppins(fontWeight: FontWeight.w600, fontSize: 13, color: AppColors.textDark),
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                  ),
                  const SizedBox(height: 4),
                  Text(
                    ...
                    style: AppFonts.poppins(fontSize: 13, fontWeight: FontWeight.bold, color: AppColors.primary),
                  ),
                ],
              ),
            ),
